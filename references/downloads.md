...
Use CC Switch for provider presets and local routing. For Codex + DeepSeek, ensure CC Switch is acting as the Responses-compatible bridge. Codex sends Responses-shaped requests; DeepSeek's public OpenAI-compatible endpoint is Chat Completions, so direct Codex-to-DeepSeek config can fail.
...
Use this when the user says "Codex++ 管理工具", "Codex++ 删除会话", "Codex++ 中转", "Codex++ 恢复官方", or wants UI enhancements without editing files by hand.
